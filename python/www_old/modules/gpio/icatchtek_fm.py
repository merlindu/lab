...
#subdir =  os.path.split(os.path.realpath(__file__))[0]
#libname = subdir + "/libicatfm_fake"
...
class Chip(object):
	...
	def __init__(self, name, nm, i2c, spi, uart):
		...
		self.pin = []
		for i in range(nm):
			self.pin.append( Pin(i, "FMGPIO"+str(i)) )
		...
		for i in range(len(i2c)):
			...
			for j in range(len(i2c[i])):
				if i2c[i][j].sda.num >= nm or i2c[i][j].sda.num >= nm:
					raise Exception("Chip initial failed, I2c arguments error: pin num beyond the range of total pin number.")

		self.spi_stat = []
		for i in range(len(spi)):
			self.spi_stat.append(0xff) #pinmux, 0xff means off
			for j in range(len(spi[i])):
				if spi[i][j].csn.num >= nm or spi[i][j].din.num >= nm or spi[i][j].dout.num >= nm or spi[i][j].clk.num >= nm or spi[i][j].bsy.num >= nm:
					raise Exception("Chip initial failed, Spi arguments error: pin num beyond the range of total pin number.")

		self.uart_stat = []
		for i in range(len(uart)):
			self.uart_stat.append(0xff) #pinmux, 0xff means off
			for j in range(len(uart[i])):
				if uart[i][j].txd.num >= nm or uart[i][j].rxd.num >= nm or uart[i][j].cts.num >= nm or uart[i][j].rts.num >= nm:
					raise Exception("Chip initial failed, Uart arguments error: pin num beyond the range of total pin number.")

	def update_pin_mode(): #function or gpio
		for i in range(self.count):
			self.pin[sda.num].name = "FUN"
		return 0
	...
	"""
	def get_i2c_status(self): #return pinmux, 0xff means off
	...
	def remove_i2c_dev(self, chnl):
		...
		sda = self.i2c[chnl][pinmux].sda
		sck = self.i2c[chnl][pinmux].sck
		self.pin[sda.num].name = "FMGPIO-"+ str(sda.num)
		self.pin[sck.num].name = "FMGPIO-"+ str(sck.num)
		self.i2c_stat[chnl] = 0xff
		return 0

	def get_spi_status(self): #return pinmux, 0xff means off
	...
	def remove_spi_dev(self, chnl):
		...
		self.pin[csn.num].name = "FMGPIO-"+ str(csn.num)
		self.pin[din.num].name = "FMGPIO-"+ str(din.num)
		self.pin[dout.num].name = "FMGPIO-"+ str(dout.num)
		self.pin[clk.num].name = "FMGPIO-"+ str(clk.num)
		self.pin[bsy.num].name = "FMGPIO-"+ str(bsy.num)
		self.spi_stat[chnl] = 0xff
		return 0

	def get_uart_status(self): #return pinmux, 0xff means off
	...
	def remove_uart_dev(self, chnl):
		...
		self.pin[txd.num].name = "FMGPIO-"+ str(txd.num)
		self.pin[rxd.num].name = "FMGPIO-"+ str(rxd.num)
		self.pin[rts.num].name = "FMGPIO-"+ str(rts.num)
		self.pin[cts.num].name = "FMGPIO-"+ str(cts.num)
		self.spi_stat[chnl] = 0xff
		return 0
